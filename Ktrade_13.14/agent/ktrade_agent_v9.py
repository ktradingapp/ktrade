...
# ── v13.12: intraday safety rules (all OFF/0 by default) ──────────────────────
def _avg_dollar_volume(df, n: int = 20) -> float:
    """Average dollar volume (close*volume) over the last n bars. 0.0 if unknown."""
    try:
        if df is None or len(df) == 0 or "close" not in df or "volume" not in df:
            return 0.0
        tail = df.tail(int(n))
        dv = (tail["close"] * tail["volume"]).mean()
        return float(dv) if dv == dv else 0.0   # NaN guard
    except Exception:
        return 0.0


def _min_avg_dollar_volume() -> float:
    """Liquidity floor (#5). 0 disables. A BUY on a name below this is downgraded."""
    try:
        return float(os.getenv("KTRADE_MIN_AVG_DOLLAR_VOL", "0"))
    except (TypeError, ValueError):
        return 0.0


def in_no_entry_window(now_et, open_block_min: int = 5, close_block_min: int = 10):
    """#13: True during the opening auction / closing rush when spreads are wide and
    prices erratic. Returns (blocked, reason). Pure — takes an ET datetime."""
    t = now_et.hour * 60 + now_et.minute
    open_start, close_end = 9 * 60 + 30, 16 * 60
    if open_start <= t < open_start + int(open_block_min):
        return True, f"opening {open_block_min}min (wide spreads)"
    if close_end - int(close_block_min) <= t < close_end:
        return True, f"closing {close_block_min}min (wide spreads)"
    return False, ""


def eod_flatten_due(now_et, cutoff_hour: int = 15, cutoff_min: int = 55) -> bool:
    """#1: True inside the end-of-day flatten window (default 15:55–16:00 ET), so an
    intraday position isn't carried overnight. Pure — takes an ET datetime."""
    t = now_et.hour * 60 + now_et.minute
    return (cutoff_hour * 60 + cutoff_min) <= t < 16 * 60
...
class ConvictionScorer:
    ...
    def score(self, ticker: str, df: pd.DataFrame, interval: str = "1d", benchmark_ret5: Optional[float] = None) -> ConvictionScore:
        ...
        # v13.12: liquidity floor (#5) — downgrade a BUY on a thin name (bad slippage
        # risk) below the configured average dollar volume. 0 disables (default).
        _min_liq = _min_avg_dollar_volume()
        if _min_liq and signal == +1:
            adv = _avg_dollar_volume(df, 20)
            components["avg_dollar_vol"] = round(adv, 0)
            if adv < _min_liq:
                log.info("LIQUIDITY skip %s: avg $vol %.0f < %.0f", ticker, adv, _min_liq)
                signal = 0
                components["liquidity_blocked"] = 1.0
...
class KTradeCEO:
    ...
    def __init__(self, broker=None, market_fn=None):
        ...
        # v13.7: N-cycle BUY confirmation (off by default; require a name to clear
        # gates in N consecutive scans before executing). Fixes one-scan blips.
        try:
            try:
            ...
            self.buy_confirm = BuyConfirmation()
        except Exception:
            self.buy_confirm = None
        self._cycle    = 0
        self._running  = False
    ...
    def _intraday_eod_flatten(self) -> list:
        """v13.12 (#1): near the close, flatten open positions when running intraday,
        so an intraday trade doesn't silently become an overnight swing exposed to
        gaps. OFF by default (KTRADE_INTRADAY_EOD_FLATTEN); only fires on intraday
        intervals inside the 15:55–16:00 ET window. Best-effort: it can only place
        RTH orders while the market is open, so a position is closed only if a cycle
        runs in that window. Mirrors the earnings-ahead exit path."""
        out = []
        if os.getenv("KTRADE_INTRADAY_EOD_FLATTEN", "false").strip().lower() != "true":
            return out
        if os.getenv("KTRADE_SCAN_INTERVAL", "1d").strip().lower() not in INTRADAY_INTERVALS:
            return out
        try:
            now_et = self.heartbeat._now_et()
        except Exception:
            return out
        if not eod_flatten_due(now_et):
            return out
        sell_fn = getattr(self.execution.monitor, "broker_fn", None)
        for tkr in list(self.risk.engine.open_positions.keys()):
            pos = self.risk.engine.open_positions.get(tkr, {})
            ...
            if qty > 0 and sell_fn:
                try:
                    ...
                    log.error("EOD flatten sell failed for %s: %s", tkr, exc)
            log.warning("EOD FLATTEN %s qty=%s (sold=%s)", tkr, qty, sold)
            out.append({"ticker": tkr, "qty": qty, "sold": sold, "reason": "intraday EOD flatten"})
        return out
    ...
    def run_cycle(self, data_map: Dict[str, pd.DataFrame],
        ...
        # v13.12 (#1): intraday end-of-day flatten (off by default; only near close).
        results["eod_flatten"] = self._intraday_eod_flatten()

        # 2. Research — rank universe
        interval = os.getenv("KTRADE_SCAN_INTERVAL", "1d").strip() or "1d"
        intraday = interval.lower() in INTRADAY_INTERVALS

        # v13.5: two-tier watchlist (off by default). On intraday cycles, restrict
        # the universe to the fresh daily-published shortlist instead of re-scanning
        # everything; on daily cycles, publish the shortlist for the intraday tier.
        _wl = None
        try:
            try:
                from agent.intraday_watchlist import IntradayWatchlist, mode_on, watchlist_size
            except Exception:
                import sys as _sys
                _here = os.path.dirname(os.path.abspath(__file__))
                if _here not in _sys.path:
                    _sys.path.insert(0, _here)
                from intraday_watchlist import IntradayWatchlist, mode_on, watchlist_size
            if mode_on():
                _wl = IntradayWatchlist()
                if intraday:
                    data_map, _why = _wl.filter_universe(data_map)
                    log.info("watchlist mode: %s", _why)
        except Exception as exc:
            log.debug("watchlist consume skipped: %s", exc)
        ...
        # v13.5: publish the daily shortlist for the intraday tier to consume.
        if _wl is not None and not intraday:
            try:
                _wl.publish([s.ticker for s in top_scores[:watchlist_size()]], source=interval)
                log.info("watchlist mode: published top %d daily names", min(len(top_scores), watchlist_size()))
            except Exception as exc:
                log.debug("watchlist publish skipped: %s", exc)

        # 3. For each top pick: strategy â†’ risk â†’ execute
        placed = 0
        _approved_this_cycle = set()                 # v13.7: names that cleared gates
        _cycle_now = datetime.now(timezone.utc)      # v13.7: for confirmation gap guard
        # v13.12 (#13): pause NEW entries during the wide-spread open/close windows
        # (off by default). Exits above still run; only new entries are held.
        _no_entry_reason = None
        if os.getenv("KTRADE_AVOID_OPEN_CLOSE_MINUTES", "false").strip().lower() == "true":
            try:
                _blocked, _why = in_no_entry_window(self.heartbeat._now_et())
                if _blocked:
                    _no_entry_reason = _why
            except Exception:
                pass
        for score in top_scores[:10]:  # evaluate top 10
            if _no_entry_reason:
                log.info("Skipping new entries: %s", _no_entry_reason)
                results["blocked"].append({"ticker": "*", "reason": f"no-entry window: {_no_entry_reason}"})
                break
            ...
            if decision.approved:
                # v13.7: N-cycle confirmation gate. Track this cycle's pass; only
                # execute once the name has cleared gates in N consecutive cycles.
                # No-op when KTRADE_BUY_CONFIRM_CYCLES=1 (default).
                if self.buy_confirm is not None:
                    _approved_this_cycle.add(score.ticker)
                    streak = self.buy_confirm.register_pass(score.ticker, _cycle_now)
                    if not self.buy_confirm.is_confirmed(score.ticker):
                        log.info("HELD for confirmation: %s (%d/%d cycles)",
                                 score.ticker, streak, self.buy_confirm.required)
                        results.setdefault("held_for_confirmation", []).append(
                            {"ticker": score.ticker, "streak": streak,
                             "required": self.buy_confirm.required})
                        continue
        ...
        # v13.7: break confirmation streaks for names that did NOT clear gates this
        # cycle, so confirmation only ever counts truly consecutive passes.
        if self.buy_confirm is not None:
            self.buy_confirm.end_cycle(_approved_this_cycle)
...
# MAIN CLI
# ===========================================================================
def _acquire_agent_lock():
    """v13.10: single-instance guard for TRADING invocations (the default
    autonomous loop and --once). Prevents two agent processes from double-trading
    if launched twice (e.g. a stray manual run alongside the systemd service).
    Read-only modes (--score-only/--risk-status/--crash-check/--ask/--validate)
    skip this. Uses the existing crash-safe ProcessLock (a stale lock from a dead
    PID is reclaimed automatically). Fails OPEN if the lock module is unavailable,
    so it can never itself prevent legitimate startup."""
    try:
        from ktrade_runtime.process_lock import ProcessLock
        try:
            from ktrade_runtime.paths import app_data_dir
            lock_path = app_data_dir() / "ktrade_agent.lock"
        except Exception:
            lock_path = project_dir / "data" / "ktrade_agent.lock"
        lock = ProcessLock(lock_path)
        if not lock.acquire():
            log.critical("Another KTrade AGENT instance is already running (%s). "
                         "Exiting to prevent double-trading.", lock_path)
            sys.exit(2)
        log.info("Agent single-instance lock acquired: %s", lock_path)
        return lock
    except SystemExit:
        raise
    except Exception as exc:
        log.warning("Agent process-lock check skipped (%s); continuing", exc)
        return None


def make_standalone_market_fn():
    """v13.14: market feed (VIX/SPY) that does NOT import the Flask backend, so the
    agent's regime protection can't fail just because dashboard deps are missing
    (the v13.13 version went through broker_adapter -> backend.ktrade_alpaca and
    broke on 'No module named ktrade_alpaca'). Uses PolygonDataFeed directly, with
    the same strict fail-safe: on any failure VIX/SPY are OMITTED (never defaulted),
    and VIX is sanity-gated to 5-150."""
    def _fn():
        out = {}
        try:
            if str(project_dir) not in sys.path:
            ...
            feed = PolygonDataFeed()
            try:
                vix = feed.get_vix(default=None)   # strict: None on failure
                if vix is not None and 5.0 <= float(vix) <= 150.0:
                    out["vix"] = float(vix)
            except Exception:
                pass
            try:
                spy_df = feed.get_bars("SPY", days=5, interval="1d")
                if spy_df is not None and len(spy_df) > 0:
                    out["spy_price"] = float(spy_df["close"].iloc[-1])
            except Exception:
                pass
        except Exception:
            pass
...
def _wire_broker():
    """v13.14: wire the Alpaca paper broker adapter into the autonomous agent when
    paper order submission is enabled. Without this, ktrade-agent.service scans and
    approves but ExecutionAgent has no broker and returns simulated/no-fill — i.e.
    it was never a true autonomous paper-trading service. Gated by
    KTRADE_PAPER_ORDER_SUBMISSION (default false = previous scan-only behaviour)."""
    if os.getenv("KTRADE_PAPER_ORDER_SUBMISSION", "false").strip().lower() != "true":
        return None
    try:
        try:
            from agent.broker_adapter import AlpacaBrokerAdapter
        except Exception:
            from broker_adapter import AlpacaBrokerAdapter  # type: ignore
        broker = AlpacaBrokerAdapter()
        log.info("Alpaca PAPER broker adapter wired (KTRADE_PAPER_ORDER_SUBMISSION=true).")
        return broker
    except Exception as exc:
        log.error("Could not wire AlpacaBrokerAdapter (%s); running scan-only", exc)
        return None
...
def main():
    ...
    parser.add_argument("--validate",    action="store_true", help="Run the validation pipeline (compile, tests, backtest, fragility)")
    ...
    parser.add_argument("--prices",      type=str,            help="Price CSV for --validate backtest/fragility stages")
    parser.add_argument("--strategy",    type=str, default="macd", help="Strategy for --validate (macd/ema/momentum)")
    args = parser.parse_args()

    # v13.10: guard the trading paths (autonomous default + --once) against a
    # double-launch. Read-only modes are exempt so you can still scan / query
    # while the loop runs.
    _trading_mode = bool(args.once) or not (
        args.score_only or args.risk_status or args.crash_check or args.ask or args.validate)
    _agent_lock = _acquire_agent_lock() if _trading_mode else None
    ...
    # v13.14: market feed is now STANDALONE (no Flask backend dependency) and the
    # PAPER broker adapter is wired when KTRADE_PAPER_ORDER_SUBMISSION=true, so the
    # autonomous service actually places paper bracket orders. Fails safe both ways.
    _market_fn = None
    if os.getenv("KTRADE_WIRE_MARKET_FN", "true").strip().lower() == "true":
        _market_fn = make_standalone_market_fn()
        log.info("Market feed wired (standalone VIX/SPY via PolygonDataFeed).")
    _broker = _wire_broker()
    ceo = KTradeCEO(broker=_broker, market_fn=_market_fn)
    ...
    if args.validate:
        # v13.11: run the real validation pipeline (v13.8) instead of the old dead
        # stub. Without --prices, the data-dependent backtest/fragility stages skip
        # cleanly; with --prices they execute on the VPS.
        import subprocess
        script = project_dir / "scripts" / "run_validation_pipeline.py"
        if not script.exists():
            print("Validation pipeline not found at scripts/run_validation_pipeline.py")
            return
        cmd = [sys.executable, str(script)]
        if args.prices:
            cmd += ["--prices", args.prices, "--strategy", args.strategy]
        print(f"Running validation pipeline: {' '.join(cmd)}\n")
        raise SystemExit(subprocess.run(cmd, cwd=str(project_dir)).returncode)
    ...
    if args.once:
        ...
        # v13.11: --once now runs the same pre-cycle guard as the autonomous loop,
        # so a stale VIX / broker desync refuses new entries here too (previously
        # --once bypassed this). In paper mode (KTRADE_REQUIRE_FRESH_VIX=false) it
        # proceeds; with no VIX feed and the default, it stays conservative.
        safe, reason = ceo._broker_truth_guard()
        if not safe:
            print(f"Pre-cycle guard: no new entries this run — {reason}")
            return
