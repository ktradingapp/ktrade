...
def make_market_fn():
    """Returns a callable producing {'vix','spy_price'} for KTradeCEO(market_fn=...).

    v13.13: VIX is now wired via the data feed (Polygon I:VIX, yfinance ^VIX
    fallback), fetched STRICTLY: if no source succeeds it returns None and VIX is
    OMITTED, so the stale-VIX guard stays conservative (fail-safe) instead of
    trusting the calm 18.0 default. The value is sanity-gated to a plausible band to
    reject bad ticks. Note: Polygon I:VIX needs an index-data subscription; if yours
    doesn't include it (and yfinance is unavailable/unreliable from the VPS), VIX
    stays unwired and the guard keeps you conservative — verify on the VPS."""
    def _fn():
        out = {}
        try:
        ...
        try:
            try:
                ...
                from ktrade_data import PolygonDataFeed  # type: ignore
            vix = PolygonDataFeed().get_vix(default=None)   # strict: None on failure
