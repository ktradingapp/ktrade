...
# KTrade v10.3 End-to-End Project Document

Project folder: C:\trading-agent\Ktrade_v10_3

This document explains how the KTrade v10.3 paper-trading project works end to end. It covers the dashboard, backend, market data, strategy engine, VectorBT backtesting, sentiment layer, broker reconciliation, risk rules, and daily run process.
...
Trend: 23%
MACD: 16%
Relative Strength: remaining small confirmation
ORB: excluded on daily because opening range needs intraday candles
```
...
```text
Price is above long-term trend
MACD turns bullish
Trend condition is supportive
```
...
```text
First part of the trading day creates a high and low range.
If price breaks above that range with volume, it may be an intraday BUY setup.
```
...
data\ktrade_approved_params.json
...
```text
VectorBT approval does not guarantee future profit.
It only says the setup passed historical rules.
```
...
```text
Recent news is positive because analysts mention strong AI demand and revenue growth.
This supports the existing BUY signal, but strategy and risk rules should still decide.
```
...
```text
VectorBT checks if strategy worked historically.
Scanner checks current market conditions.
Finnhub gets news.
FinBERT scores financial sentiment.
OpenAI explains the sentiment.
Dashboard shows BUY/WATCH with reasoning.
Alpaca places paper trades.
Broker reconciliation checks the truth.
...
BUY threshold: 75
Sentiment: Finnhub + FinBERT + OpenAI available when keys/packages are configured
```
