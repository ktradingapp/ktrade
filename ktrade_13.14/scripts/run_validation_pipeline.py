...
def stage_tests():
    ...
    # v13.14: per-test timeout so one slow/hanging suite (e.g. a test that itself
    # spawns many subprocesses) fails LOUDLY as TIMEOUT instead of stalling the
    # whole pipeline. Override with KTRADE_PIPELINE_TEST_TIMEOUT_S.
    per_test_timeout = float(os.getenv("KTRADE_PIPELINE_TEST_TIMEOUT_S", "300"))
    failed = []
    for t in suites:
        rc, out = _run([_py(), t], env=env, timeout=per_test_timeout)
        if rc == 124:
            failed.append(f"{os.path.basename(t)} TIMEOUT(>{per_test_timeout:.0f}s)")
        elif rc != 0:
