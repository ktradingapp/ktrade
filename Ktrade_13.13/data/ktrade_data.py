...
class PolygonDataFeed:
    ...
    def get_vix(self, default=18.0):
        """Fetch current VIX level. Returns `default` if no source succeeds.
        Pass default=None for strict callers (e.g. the market feed) that must
        distinguish a real reading from a fallback so they can fail safe."""
        if POLYGON_KEY:
        ...
        if YFINANCE_OK:
            try:
                ...
                pass
        return default  # 18.0 by default; None for strict callers so they fail safe
