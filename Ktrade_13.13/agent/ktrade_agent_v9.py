...
def main():
    ...
    # v13.13: wire the market feed (VIX/SPY) so the VIX ladder + stale-VIX guard use
    # real data instead of the dormant 18.0 default. Fails safe: if VIX can't be
    # fetched it's omitted and the guard stays conservative. Disable with
    # KTRADE_WIRE_MARKET_FN=false.
    _market_fn = None
    if os.getenv("KTRADE_WIRE_MARKET_FN", "true").strip().lower() == "true":
        try:
            try:
                from agent.broker_adapter import make_market_fn
            except Exception:
                from broker_adapter import make_market_fn  # type: ignore
            _market_fn = make_market_fn()
            log.info("Market feed wired (VIX/SPY via data feed).")
        except Exception as exc:
            log.warning("Could not wire market_fn (%s); VIX guards stay conservative", exc)
    ceo = KTradeCEO(market_fn=_market_fn)
