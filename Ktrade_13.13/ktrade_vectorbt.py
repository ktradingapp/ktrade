...
class ParameterOptimizer:
    ...
    def __init__(self, cash=100_000, fees=0.001, slippage=0.0005):
        self.cash = cash
        self.fees = fees
        # v13.11: model slippage on top of fees so backtest fills aren't optimistic
        # (KTRADE_BACKTEST_SLIPPAGE overrides; ~5bps default).
        import os as _os
        self.slippage = float(_os.getenv("KTRADE_BACKTEST_SLIPPAGE", str(slippage)))
...
class WalkForwardValidator:
    ...
    def __init__(self, cash=100_000, fees=0.001, n_splits=4, slippage=0.0005):
        self.cash     = cash
        self.fees     = fees
        import os as _os
        self.slippage = float(_os.getenv("KTRADE_BACKTEST_SLIPPAGE", str(slippage)))
        ...
        for i in range(self.n_splits):
            ...
            try:
                ...
                pf = vbt.Portfolio.from_signals(
                    ...
                    init_cash=self.cash, fees=self.fees, slippage=self.slippage, freq="1D",
                )
                s = pf.stats()
                def _sf(v, d=0.0):
                    try:
